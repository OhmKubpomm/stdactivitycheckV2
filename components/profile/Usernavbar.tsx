        ...
        {menus?.map((menu, i) => (
         ...
         ${menu?.gap ? "mt-5" : ""}`}
          >
            ...
            {!open && (
              <h2 className="absolute left-14 bg-white font-medium whitespace-pre text-gray-900 rounded-md drop-shadow-lg px-0 py-0 w-0 overflow-hidden group-hover:px-2 group-hover:py-1 group-hover:duration-300 group-hover:w-fit">
                {menu?.name}
              </h2>
            )}
          </Link>
